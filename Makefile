...
ifeq ($(GBWT_ENABLE_SHARED_MEMORY), 1)
    ...
    # sdsl-lite's int_vector only exposes the shared-memory constructor we
    # use when it sees this defined too (see SDSL_ENABLE_SHARED_MEMORY in
    # $(SDSL_DIR)/Make.helper, which we already checked above is ON).
    DEFINES += -DSDSL_ENABLE_SHARED_MEMORY
...
Make.helper: .FORCE
	@echo "# Generated by ../Makefile; do not edit, and do not commit to git." > $@
	@echo "MY_CXX=$(MY_CXX)" >> $@
	@echo "MY_CC=$(MY_CC)" >> $@
